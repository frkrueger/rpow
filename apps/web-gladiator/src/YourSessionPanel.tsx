...
export function YourSessionPanel({ session, onClosed }: Props) {
  ...
  const flipsRemaining = bet > 0n ? remaining / bet : 0n;

  return (
    <div className="panel">
      <h2>YOUR SESSION</h2>
      <div style={{ marginTop: 8 }}>
        Bankroll: <strong>{formatRpow(session.bankroll_remaining_base_units)} RPOW</strong> ({flipsRemaining.toString()} flips at {formatRpow(session.bet_base_units)} RPOW each)
      </div>
      <div style={{ marginTop: 4, fontSize: 12, color: '#aaa' }}>
        W/L this session: <strong>{session.flips_won}</strong> / <strong>{session.flips_lost}</strong>
      </div>
      <button
        style={{ marginTop: 12 }}
        onClick={onLeave}
