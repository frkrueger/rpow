...
// ---------------------------------------------------------------------------
// GET /api/gladiator/chat
// ---------------------------------------------------------------------------

describe('GET /api/gladiator/chat', () => {
  ...
  it('200 empty when no messages', async () => {
    ...
    const res = await ctx.app.inject({ method: 'GET', url: '/api/gladiator/chat' });
  ...
  it('returns up to 50 messages', async () => {
    const ctx = await makeTestApp(); cleanup = ctx.cleanup;
    await login(ctx, 'alice@b.com');
    await markVerified(ctx.pool, 'alice@b.com', 'alice');

    for (let i = 0; i < 55; i++) {
      await insertChatMessage(ctx.pool, 'SYSTEM', `msg ${i}`);
    }
  ...
  it('includes SYSTEM rows alongside USER rows', async () => {
    const ctx = await makeTestApp(); cleanup = ctx.cleanup;
    await login(ctx, 'alice@b.com');
    await markVerified(ctx.pool, 'alice@b.com', 'alice');

    await insertChatMessage(ctx.pool, 'USER', 'user message', 'alice@b.com', 'alice');
    await insertChatMessage(ctx.pool, 'SYSTEM', '@alice entered the arena');
    ...
    const kinds = body.messages.map((m: any) => m.kind);
    expect(kinds).toContain('USER');
    expect(kinds).toContain('SYSTEM');

    // SYSTEM rows have null account_email
    const systemMsg = body.messages.find((m: any) => m.kind === 'SYSTEM');
    expect(systemMsg.account_email).toBeNull();
    expect(systemMsg.x_handle).toBeNull();
  });
  ...
  it('before param paginates correctly', async () => {
    ...
    const id1 = await insertChatMessage(ctx.pool, 'SYSTEM', 'older message');
    // Wait a tiny bit to ensure different timestamps
    const olderRes = await ctx.pool.query<{ created_at: Date }>(
      `SELECT created_at FROM gladiator_chat_messages WHERE id = $1`, [id1],
    );
    ...
    await ctx.pool.query(
      `INSERT INTO gladiator_chat_messages (id, account_email, x_handle, kind, body, created_at)
       VALUES ($1, NULL, NULL, 'SYSTEM', 'newer message', $2::timestamptz)`,
      [randomUUID(), newerTs.toISOString()],
    );

    // Use before = 2 seconds after the older to get only older
    const cutoffTs = new Date(olderTs.getTime() + 500).toISOString();
    const res = await ctx.app.inject({
      method: 'GET',
      url: `/api/gladiator/chat?before=${encodeURIComponent(cutoffTs)}`,
    });
    expect(res.statusCode).toBe(200);
    const body = res.json();
  ...
  it('400 BAD_REQUEST for non-ISO before param', async () => {
    ...
    const res = await ctx.app.inject({
      method: 'GET',
      url: '/api/gladiator/chat?before=not-a-date',
    });
...
// ---------------------------------------------------------------------------
// POST /api/gladiator/chat
// ---------------------------------------------------------------------------

describe('POST /api/gladiator/chat', () => {
  let cleanup: (() => Promise<void>) | null = null;
  afterEach(async () => {
    if (cleanup) await cleanup();
    cleanup = null;
  ...
  it('200 happy path: row inserted with kind=USER, account_email and x_handle snapshot', async () => {
    ...
    const { rows } = await ctx.pool.query<{
      ...
      `SELECT id, account_email, x_handle, kind, body
       FROM gladiator_chat_messages WHERE id = $1`,
      [responseBody.id],
    );
    ...
    expect(rows[0].body).toBe('Hello arena!');
  });
});
