...
export async function marketRoutes(app: FastifyInstance) {
  ...
  // POST /market/list — authenticated seller lists one of their VALID tokens
  app.post('/market/list', async (req, reply) => {
    ...
    if (!owned.rows[0]) {
      return reply.code(400).send({ error: 'TOKEN_NOT_OWNED_OR_INVALID' });
    }

    const existing = await app.pool.query(
      `SELECT id FROM listings WHERE token_id = $1 AND status = 'active'`,
      [token_id],
    );
    if (existing.rows[0]) {
      return reply.code(400).send({ error: 'TOKEN_ALREADY_LISTED' });
    }

    await app.pool.query(
      `INSERT INTO listings (id, token_id, seller_email, price_rpow)
       VALUES ($1, $2, $3, $4)`,
      [randomUUID(), token_id, seller_email, price_rpow],
    );

    return reply.send({ ok: true });
  });

  // POST /market/cancel — authenticated seller cancels their own listing
  app.post('/market/cancel', async (req, reply) => {
    ...
    await app.pool.query(
      ...
      [parsed.data.listing_id, s.email],
    );
