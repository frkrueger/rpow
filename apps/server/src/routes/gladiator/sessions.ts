...
/**
 * Format a base-unit bankroll amount as a human-readable RPOW string.
 * If it divides evenly into whole RPOW, show an integer; otherwise show
 * a decimal with trailing zeros trimmed.
 */
function formatRpow(baseUnits: bigint): string {
  ...
  // Up to 9 decimal places, trailing zeros trimmed
...
export async function sessionsRoutes(app: FastifyInstance) {
  // --------------------------------------------------------------------------
  // POST /api/gladiator/sessions — Enter the arena
  // --------------------------------------------------------------------------
  app.post('/api/gladiator/sessions', {
    config: {
      rateLimit: {
        max: 5,
        timeWindow: '1 minute',
    ...
    const minBet = BigInt(app.config.gladiatorMinBetBaseUnits);
    const maxBet = BigInt(app.config.gladiatorMaxBetBaseUnits);
    const maxBankroll = BigInt(app.config.gladiatorMaxBankrollBaseUnits);
    ...
    try {
      result = await withTx<EnterResult>(app.pool, async (c) => {
        ...
        const chatBody = `@${xHandle} entered with ${formatRpow(bankroll)} RPOW`;
        await c.query(
  ...
  // --------------------------------------------------------------------------
  // POST /api/gladiator/sessions/:id/close — Leave the arena
  // --------------------------------------------------------------------------
  app.post('/api/gladiator/sessions/:id/close', {
    config: {
      rateLimit: {
        max: 10,
        timeWindow: '1 minute',
    ...
    // We also need the x_handle for the chat message
    ...
    try {
      result = await withTx<CloseResult>(app.pool, async (c) => {
        ...
        // Insert system chat message
        const handle = xHandle ?? email;
        const chatBody = `@${handle} left the arena`;
        await c.query(
          `INSERT INTO gladiator_chat_messages (id, account_email, x_handle, kind, body)
           VALUES ($1, NULL, NULL, 'SYSTEM', $2)`,
          [randomUUID(), chatBody],
